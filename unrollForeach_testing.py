        ...
        if(block.__contains__("tikzpicture")):
            for codeInsideTikzPicture in (re.split("\\\\begin{tikzpicture}", block)):
                ...
                if(i%2==1):
                    ...
                    j = 0
                    while(os.path.exists("test_"+ filename + "t" + str(j) + "_unrolled.tex")):
                        j+=1
                    with open("test_"+ filename + "t" + str(j) + "_unrolled.tex", "w") as outputFile:
                        outputFile.write(unrolledForeachInTikzPart)

def cleanup():
    filename="edge-editing-v2.tex"
    for i in range(8):
        os.system("rm "+"test_"+ filename + "t" + str(i) + "_unrolled.tex")
